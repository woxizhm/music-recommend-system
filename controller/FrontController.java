...
public class FrontController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("pc/tuijian")
    public String tuijian(Model model, HttpSession session) {
        if (session.getAttribute("loginUserinfoID") == null) {
            return "pc/login";
        }
        ...
        if (StringUtils.isEmpty(role)) {
            model.addAttribute("errormsg", "用户必须选择用户标签以后才能进行音乐推荐");
            return "redirect:userinfoCenter";
        }
        String[] names = role.split(",");
        List<Article> articleList = articleMapper.selectTuijian(names);
        model.addAttribute("beanDataList", articleList);
        model.addAttribute("usreTags", role);
        return "pc/tuijian";
    }
    ...
    public String articleDetail(Integer id, Model model, HttpSession session) {
    ...
    @RequestMapping("pc/echartsWenjuan")
    public String echartsWenjuan(int id, Model model) {
        model.addAttribute("id", id);
        return "pc/echartsWenjuan";
    }
    ...
    @ResponseBody
    @RequestMapping("pc/shoucang")
    public ResultUtil shoucang(HttpSession session, int id, String vercode) {
        Article beanData = articleMapper.selectArticleById(id);
        int userInfoID = getLoginUserInfoID(session);
        Userinfo userinfo = userinfoMapper.selectUserinfoById(userInfoID);
        return ResultUtil.ok("收藏成功");
    }
